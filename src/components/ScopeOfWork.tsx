...
export const ScopeOfWork: React.FC<ScopeOfWorkProps> = ({ onSelectService, theme = 'dark' }) => {
  ...
  const scopeList: ExtendedScopeItem[] = [
    {
      ...
      themeColor: {
        accent: 'yellow',
        badgeBgLight: 'bg-yellow-100/90 text-yellow-900 border-yellow-300',
        badgeTextLight: 'text-yellow-900',
        badgeBgDark: 'bg-yellow-950/60 text-yellow-300 border-yellow-500/30',
        badgeTextDark: 'text-yellow-300',
        ...
        iconBgLight: 'bg-yellow-50 text-amber-600 border-yellow-200/80',
        iconBgDark: 'bg-yellow-950/50 text-yellow-400 border-yellow-500/40 shadow-[0_0_14px_rgba(234,179,8,0.28)]',
      },
      features: [
    ...
    },
    {
      ...
      themeColor: {
        accent: 'blue',
        badgeBgLight: 'bg-blue-100/90 text-blue-900 border-blue-200',
        badgeTextLight: 'text-blue-900',
        badgeBgDark: 'bg-blue-950/60 text-blue-300 border-blue-500/30',
        ...
        iconBgDark: 'bg-blue-950/50 text-blue-400 border-blue-500/40 shadow-[0_0_14px_rgba(59,130,246,0.28)]',
      },
      features: [
    ...
    },
    {
      ...
      themeColor: {
        accent: 'amber',
        badgeBgLight: 'bg-amber-100/90 text-amber-800 border-amber-200',
        badgeTextLight: 'text-amber-800',
        badgeBgDark: 'bg-amber-950/60 text-amber-300 border-amber-500/30',
        ...
        iconBgDark: 'bg-amber-950/50 text-amber-400 border-amber-500/40 shadow-[0_0_14px_rgba(245,158,11,0.28)]',
      },
      features: [
    ...
    },
    {
      ...
      themeColor: {
        accent: 'emerald',
        badgeBgLight: 'bg-emerald-100/90 text-emerald-900 border-emerald-200',
        badgeTextLight: 'text-emerald-900',
        badgeBgDark: 'bg-emerald-950/60 text-emerald-300 border-emerald-500/30',
        ...
        iconBgDark: 'bg-emerald-950/50 text-emerald-400 border-emerald-500/40 shadow-[0_0_14px_rgba(16,185,129,0.28)]',
      },
      features: [
    ...
    },
    {
      ...
      themeColor: {
        accent: 'teal',
        badgeBgLight: 'bg-teal-100/90 text-teal-900 border-teal-200',
        badgeTextLight: 'text-teal-900',
        badgeBgDark: 'bg-teal-950/60 text-teal-300 border-teal-500/30',
        ...
        iconBgDark: 'bg-teal-950/50 text-teal-400 border-teal-500/40 shadow-[0_0_14px_rgba(20,184,166,0.28)]',
      },
      features: [
  ...
  return (
    ...
    >
          ...
          {scopeList.map((item, scopeIdx) => (
            <ScrollReveal key={item.id} delay={scopeIdx * 100}>
              <TiltCard
                ...
                scale={1.025}
                perspective={1000}
                className={`relative rounded-2xl p-6 sm:p-8 border-2 transition-colors duration-300 ease-out active:scale-[0.99] flex flex-col md:flex-row items-start md:items-center justify-between gap-6 group cursor-pointer overflow-hidden ${
                  isLight
                    ? `bg-slate-50/90 hover:bg-white border-slate-200/90 shadow-sm ${item.themeColor.hoverBorderLight} ${item.themeColor.hoverShadowLight}`
                    : `bg-navy-900/75 backdrop-blur-xs hover:bg-navy-850/90 border-white/15 shadow-lg ${item.themeColor.hoverBorderDark} ${item.themeColor.hoverShadowDark}`
                }`}
                ...
                <div className="flex items-start sm:items-center gap-4 sm:gap-6 relative z-10 w-full md:w-auto">
                  ...
                  <span
                    className={`text-3xl sm:text-4xl font-extrabold font-mono shrink-0 transition-all duration-300 ${
                      isLight
                        ? 'text-navy-900/30 group-hover:text-navy-900'
                        : 'bg-gradient-to-br from-amber-300 via-gold-400 to-orange-400 bg-clip-text text-transparent group-hover:from-amber-200 group-hover:via-gold-300 group-hover:to-orange-300 opacity-90 group-hover:opacity-100'
                    }`}
                  >
                    {item.numberStr}
                  </span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3
                      className={`text-xl sm:text-2xl font-extrabold transition-colors duration-300 ${
                        isLight ? 'text-navy-900 group-hover:text-navy-950' : 'text-white group-hover:text-white'
                      }`}
                    >
                      {item.title}
                    </h3>
                      ...
                      {item.tags.map((tag, tIdx) => (
                        <span
                          ...
                          className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full transition-colors duration-300 ${
                            isLight
                              ? 'bg-slate-200/80 text-slate-700 group-hover:bg-white group-hover:shadow-xs group-hover:border group-hover:border-slate-200'
                              : 'bg-navy-950/80 text-slate-300 border border-white/10 group-hover:border-white/20 group-hover:text-white'
                          }`}
                        >
                          {tag}
                        </span>
      ...
      {selectedScope && (
        <div className="fixed inset-0 z-50 bg-navy-950/75 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className={`rounded-2xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto border ${
            isLight
              ? 'bg-white border-slate-200 text-slate-800'
              : 'bg-navy-900/85 backdrop-blur-xl border-white/20 text-white'
          }`}>
            ...
            <button
              ...
              className={`relative overflow-hidden cursor-pointer absolute top-4 right-4 p-2 rounded-full transition-colors border ${
                isLight
                  ...
                  : 'bg-navy-950/70 hover:bg-navy-800 text-slate-300 border-white/15 backdrop-blur-xs'
              }`}
            ...
            <div className="space-y-4">
              ...
              <div className={`space-y-2.5 p-4 rounded-xl border ${
                isLight
                  ...
                  : 'bg-navy-950/70 backdrop-blur-xs border-white/10 text-slate-200'
              }`}>
